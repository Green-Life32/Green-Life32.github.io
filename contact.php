...
$destinataire='contactsgreenlife@gmail.com';  
...
if (isset($_POST['message']))  
  {  
    ...
    if((empty($message))or(empty($objet))or(!preg_match($verif,$email)))  
      {  
        //les 3 champs sont vides  
        if(empty($email)and(empty($message))and(empty($objet)))  
          {  
            echo '<p>Tous les champs sont vides.</p>';  
            $message='';$email='';$objet='';$apercu_resultat='';  
          }  
...
<form id='contact' method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">  
  ...
  <input name="votremail" tabindex="20" size="30" type="text" id="mail" value="<?php echo $email; ?>"></label></p>  
